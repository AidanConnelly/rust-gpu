...
impl GivesFinalTransform for (i32, Transform2D) {
    fn get_final_transform(&self, raw_data: &[u8]) -> Mat3 {
        if self.1.parent_offset == 0 {
            ...
            self.1.own_transform.mul_mat3(&((parent_index as i32, index_to_transform(parent_index as usize, raw_data)).get_final_transform(raw_data)))
        }
    }
}
...
pub fn main_cs(
    ...
    let final_transform = (index as i32, index_to_transform(index, raw_data)).get_final_transform(raw_data);
    *output_position = final_transform.mul_vec3(Vec3::new(0.1, 0.2, 0.3)).extend(0.0);
}
